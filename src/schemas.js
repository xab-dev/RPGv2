...
function validerVisuel(entry, catalogs, path) {
  ...
  // `{ <direction>: { <piece>: null | { dx?, dy?, echelle_x?, echelle_y?, echelle?, cisaillement?, pivot_y?, courbure?, longueur?, miroir?, rabat? } } }`
      ...
      for (const [direction, poses] of Object.entries(o)) {
        ...
        for (const [piece, pose] of Object.entries(poses)) {
          ...
          if (!cles || rabatMalForme || cles.some((c) => c !== 'miroir' && c !== 'rabat' && !positifs.includes(c) && !nombres.includes(c))
            ...
            erreurs.push(`${cheminO} > ${piece} doit être null (cachée) ou { dx?, dy?, cisaillement?, pivot_y?, courbure?, rotation? : nombres ; echelle_x?, echelle_y?, echelle?, longueur? : nombres > 0 ; miroir? : booléen ; rabat? : { y, longueur > 0, hauteur > 0 } }`);
