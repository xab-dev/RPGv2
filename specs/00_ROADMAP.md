...
### Ordre d'injection (2026-09-19, soir) — un ticket par session, un commit par ticket

Source : `NS_decisions-rendu-navigateurs_2026-09-19.md` §5, qui **remplace** le §5 de `NS_decisions-fondations_2026-09-19.md` (archivée). Principe de Xav inchangé : **les fondations d'abord**, on ne rajoute pas de contenu sur des bases non confirmées. Validation en jeu de Xav entre deux tickets.

1. Session de documentation « rendu, navigateurs, téléphone » (doc seule) — **faite**.
2. **Relevés de base avant contenu** (`A-03`) — **action de Xav** : Chrome, plein écran, manette, protocole de traversée, un relevé de **jour** (`R-14`) et un de **nuit** (`R-03`). C'est le point de comparaison de `07` : sans lui, on accusera les monstres à tort, ou on les innocentera à tort.
3. `D-17` — bouton MENU tactile sous le bandeau.
4. `D-30` — plein écran demandé au premier appui tactile (paysage verrouillé si possible). Même périphérique et même validation que `D-17` : à traiter avec lui. Fait passer l'A04 de 46 % à presque tout l'écran, et de l'échelle 3 à 4.
