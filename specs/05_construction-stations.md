...
- **Entrée** : menu DOM « Construction » (`creerControleurMenu()`), disponible depuis `MENU` **seulement quand le héros est dans la structure maison** (sinon l'entrée n'apparaît pas). Liste des stations `placable` de cette structure.
- **Mode construction** : choisir une station → le jeu reste gelé comme sous UI (survie, cooldowns, jour/nuit) ; un **fantôme** de la station apparaît à sa position courante sur la **grille intérieure**. `MOVE` déplace le fantôme **d'une tuile par front montant** (snap, jamais en px) ; un verbe fait tourner de **90°** ; confirmer = pose ; annuler = retour sans changement. Mapping identique au menu manette acté : confirmer `ATTACK`, annuler `SKILL_3`/B. Les touches du mode sont affichées dans le menu lui-même (c'est une UI, pas du gameplay → pas via `hints.js`).
...
src/ui/menu.js            + entrée Construction (contextuelle)
