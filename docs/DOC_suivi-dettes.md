...
| D-20 | **« Mains nues » = première arme du jeu** (décision `Q-21`). La portée de l'auto-attaque de base passe à **la moitié de l'actuelle** (provisoire), portée par l'entrée d'arme, jamais par une stat ni une constante de `combat.js`. La case jaune de la barre du bas dessine l'**icône de l'arme équipée** : une main | Un seul objet en données : portée + icône. L'anneau de feedback dérive de la même valeur. Deux paliers, un par session : **A** données + portée (validation : la Grotte, premier monstre) · **B** icône de la case. Ticket : `MT_mains-nues_2026-09-19.md` | P2 | ouvert |
| D-21 | **Rayon d'effacement du toit : −10 %** (retour de playtest du 19/09) | Une valeur (facteur du halo, 1,25 → 1,125, *provisoire*), déjà notée « à équilibrer au ressenti » depuis le 16/09. Ticket : `MT_toit-rayon_2026-09-19.md` | P2 | ouvert |
| D-22 | **Clavier : inverser E et F.** `E` → `INTERACT` (interagir, récolter), `F` → `CONSUME` (se nourrir, utiliser le fruit équipé) | Couche d'input seule. À vérifier : `F` libre ; glyphes et indices de commande **dérivés** du mapping (sinon : une seule source, et le signaler). Ticket : `MT_clavier-e-f_2026-09-19.md` | P2 | ouvert |
