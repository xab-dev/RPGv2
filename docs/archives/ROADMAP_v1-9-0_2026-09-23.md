...
# RPG V2 — La ROADMAP d’origine (v1.9.0, 23/09), archivée le 25/09

Remplacée par `specs/00_ROADMAP.md` v2.0.0 (décision de Xav, `Q-175` : « une v2, et archiver l’originale »). Recopiée **telle quelle** : le document de cadrage de départ, les phases 0 à 7 telles qu’elles étaient pensées, l’ordre d’injection des 19-20/09. Ce qui y était encore en vigueur est repris, à jour, dans la v2 ; une valeur de la v2 qui diffère de celle-ci la **révise** (la v2 le dit).
