...
| `cf78732` | DOC | Xav : « go pour le polish -> C puis si tous les tests passent, fusionne et push ». « Où on en est » : `v0.9.28`. Suivent la fusion dans `main` et la version (tag `v0.9.28`, poussés). Palier D reste à faire, sur `audit-heros` |
| `(ce commit)` | Palier D | **L'état des lieux.** `tools/scenarios/audit_2026_09_26.mjs`, rejouable par parties, et `docs/captures/Audit_2026_09_26/` (50 images, 14 Mo, `LISEZ-MOI.md`). La vue dézoomée est dessinée par le jeu lui-même : `?echelle=1`, une fenêtre de la taille de la scène, `RESOLUTION_LOGIQUE` élargie depuis la page (la caméra, le calque, le voile et le champ la relisent à chaque frame). Constats proposés, rien corrigé : `D-295` (les teintes des follets recopiées dans l'atelier et le banc du tour ont dérivé ; l'album passe celles du catalogue), `Q-181` (le standing du décor et des monstres face au héros), `Q-182` (les grands vides : l'est de la carte, l'intérieur de la maison), `Q-183` (le damier de l'herbe). La loupe de `commun.mjs` garde le jeu autour d'elle : l'audit a la sienne, qui écrit l'image seule |
