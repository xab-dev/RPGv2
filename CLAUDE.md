...
- **Branches ouvertes** : la nuit du héros (26/09, autonomie de Xav) — `heros-simplification` → `heros-polish` → `heros-animation`, chacune partie de la précédente, rien fusionné ni poussé. Le journal de la nuit dit l'état. À reprendre aussi : `D-261` (le polish de profil).
