...
- **Le budget de la carte se mesure à chaque agrandissement** (`specs/13` palier F, 24/09 ; décision de Xav avec la spec). *Révisé le 25/09 par Xav (`Q-156`) : trois niveaux, selon ce que le ticket touche.* **Le banc complet** — `tools/scenarios/cout_calque.mjs` et `traversee_nuit.mjs` (Chrome sans fenêtre, 1920 × 1080, Bas, Moyen et Haut, ×1 et ×6 ; ~25 min) — pour les **gros patchs** (la carte Maison agrandie ou peuplée : monstres, interactifs, lumières, décor ; le rendu partagé : `render.js`, calque, obscurité), et **une fois par jour**, en fin de journée ou au réveil. **La sentinelle** — `traversee_nuit.mjs` en Moyen, ×1 et ×6 (~5 min ; la nuit est le cas le plus chargé) — pour tout ticket qui touche le **code joué à chaque frame dans toutes les scènes** (`maj()`, `dessiner()`, les conditions, les systèmes appelés par eux) : c'est là que la régression du palier B de la spec 14 s'est vue. Au-delà de 10 % sur un chiffre, le banc complet. **Rien** pour un ticket qui ne touche que des **données** (contenu, équilibrage : Xav retouche lui-même `data/`). Les chiffres se comparent à ceux du dernier banc complet (à défaut, ceux de clôture de la spec 13, `specs/13` §7, palier F). Une régression de plus de **20 %** *(provisoire)* sur le coût d'une reconstruction, `maj()`, `dessiner()` ou les frames lentes arrête le ticket et remonte à Xav. Le contrat tenu par `tests/test_budget_carte` : une frame et une reconstruction dépendent de ce qui est à l'écran, jamais de la taille de la scène. Seule l'entrée en scène peut suivre la taille de la carte, sous un plafond en données (`graphismes.json > budget_carte`, `Q-134`) ; un dépassement s'écrit en console sous `?debug=fps`.
