...
- **Branches ouvertes** : la nuit du héros (26/09), rien fusionné ni poussé — `heros-simplification` (`D-262`, `poses.js`) → `heros-polish` (`D-263` à `D-267`) → `heros-animation` (**spec 16**, paliers A à C et `D-271` ; `V-182` à `V-190` **validés** par Xav le 26/09). **En cours** sur `heros-animation` : spec 16 palier D (`D-273`, le pas qui suit la vitesse, sous 3 Hz). À reprendre : `D-261` (le profil, à dire par Xav), `D-272`, `D-274` (hors spec, depuis `main`).
