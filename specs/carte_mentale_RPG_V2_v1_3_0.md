...
**Version : 1.3.0** — **cadrage clos. Roadmap rédigée : `specs/00_ROADMAP.md`, Phase 0 détaillée dans `specs/01_socle-technique.md`.**

**Changelog depuis 1.2.0 — reports documentaires du 2026-09-16 (`specs/MT_reports-documentaires_2026-09-16.md`), sans aucune ligne de code** : fichier renommé (`v1_2_0` → `v1_3_0`). Décisions et validations en jeu des sessions 2026-09-15/16 reportées ici : mapping manette **validé à la manette réelle** (C3③), hot-swap validé (C3⑦), résolution 480×270 + rendu net **validés** (C7②), table des synergies **écrite et actée** (D6③, débloque D2③), portée d'arme = donnée (D3②), `rayon_lumiere` du follet validé (D11), formule de survie actée (D19①), direction artistique (catalogue `visuels.json`) **validée en jeu** (P4), cinématique d'intro livrée, feedback de combat et anti-spam dialogue livrés (Phase 1b). Dette tactile assumée jusqu'aux compétences (C3②). Compteur d'avancement recompté (§ légende).
...
**Avancement : 31 🟢 / 7 🟡 / 0 ⚪ / 1 🔵** (recompté le 2026-09-16 : la table des synergies élémentaires, D6③, était la 8ᵉ entrée 🟡 — elle est désormais écrite et actée, donc 🟢)

Les 7 🟡 restants ne sont plus des modules non traités : ce sont les 7 modules CORE encore ouverts (C3, C5, C6, C7, C8, C10, C11), des sous-branches de second ordre (valeurs d'équilibrage, formats de fichier précis, listes de contenu) qui se spécifient **dans** le cahier des charges, pas avant lui. Plus aucune exception bloquante.
...
|---|---|---|---|---|---|
| **Grotte** | Donjon-tutoriel en 2 salles | Cinématique courte non narrée (grotte-puits, roche, obscurité, un rayon de lumière). 3 feux follets animés en survol → **choix** (Feu/Eau/Terre), les deux autres partent. Salle 1 : levier qui s'allume sans effet (préfiguration), sortie latérale. Salle 2 : un monstre apparaît → tuto attaque de base + lore du follet ; puis **3 leviers sans instruction** (milieu → gauche → droite), une porte apparaît. | Sortie directe sur la Région Maison — la grotte est au fond du jardin | Mystérieux, « vu et revu » assumé | 2 salles |
| **Région Maison — extérieur** | Grande carte de tuiles | Forêt (bois), pierre à miner, fruits, **champs à semer**, animaux qui réapparaissent (loups → viande, lapins…), **déco plantable** (arbres, fleurs). Cycle jour/nuit, le **follet est la seule lumière**. Pêche : reportée. | Accès à la 1ère zone gaté par **niveau ~5** (XP de récolte et de craft) | Chill, lumineux | ≥ 5400 × 3700 |
...
|---|---|---|---|
| C3 | **Input abstrait** | 🟡 | ① liste définitive des verbes (`MOVE ATTACK SKILL DODGE INTERACT MENU` + ?) · ② mapping tactile (dette assumée jusqu'aux compétences, Phase 4 — 2026-09-16) · **✅③ mapping manette : ABXY = attaque + 3 skills, gâchette = consommable, LB = interact, Start = menu (n'ouvre qu'un menu fermé), B = retour dans toute UI ; croix directionnelle réservée à de futures actions secondaires, jamais `MOVE` — validé à la manette réelle le 2026-09-16** · ④ mapping clavier · ⑤ remapping par le joueur ? · ⑥ détection auto du périphérique · **✅⑦ hot-swap manette ↔ clavier — validé en jeu le 2026-09-15** |
...
| C7 | **UI / HUD** | 🟡 | ⓪ **la manette est la référence, le tactile l'adaptation** (C0) · **✅① paysage** (déjà acté `02_grotte.md` §2.2) · **✅② résolution de référence + scaling : 480 × 270 logique, cadrage à échelle entière, rendu net à la résolution physique (canvas hors-écran en pixels physiques, DPR pris en compte) — le jeu n'est pas en pixel art (style « assemblage moderne »), validé sur capture le 2026-09-15, rendu net validé le 2026-09-16** · ③ zones de pouce / safe areas (encoches) · ④ taille minimale des cibles tactiles · ⑤ structure des menus (inventaire, carte, journal) · ⑥ feedback haptique · ⑦ règle « zéro pop-up commercial » actée |
...
| D2 | **Éléments & synergies** | 🟡 | ✅ nombre = 3 (Feu/Eau/Terre), table extensible en données · **✅③ définition d'une synergie : un effet sur le joueur + un effet sur le monstre engagé dans l'aura, par élément, en données (`synergies.json` + `status_effects.json`) — débloqué par D6③, 2026-09-15** · ④ ce que le choix initial verrouille vraiment · ⑤ correspondance avec les 9 éléments haTD (voir §0bis) · ⑥ rendu visuel d'un élément sans dépendre de la couleur seule (voir P4) · ✅⑦ scaling via équipement + affinité + maîtrise, tout converge sur Force · ⑧ comment la maîtrise élémentaire monte exactement |
| D3 | **Équipement / armes** | 🟡 | ✅① 3 slots : arme, armure, accessoire · ② schéma d'une arme (`name type damage element speed abilities requirements effects` **+ `portee {min, max}` en tuiles — la portée de l'auto-attaque vient de l'arme, jamais d'une stat**, ex. épée en bois `[0;1]`, épée en pierre `[0;1.1]`, arc `[2;4]`, décision Xav 2026-09-15 ; le reste du schéma reste 🟡 pour la Phase 4) · ③ raretés · ④ upgrade / enchantement · ⑤ sets · ⑥ conditions de port · ⑦ apparence liée ou cosmétique séparée |
...
| D6 | **Effets d'état** | 🟡 | ✅① les 3 familles, dérivées logiquement de l'élément · ② durée / stacking / refresh · **✅③ table des synergies élémentaires : écrite et actée le 2026-09-15 (`02_grotte.md` §3.4), valeurs provisoires à équilibrer en jeu — débloque D2③** · ④ affichage HUD avec 5 slots d'action déjà occupés · ⑤ source (repas, compétence, environnement) |
...
| D11 | **Compagnons** | 🟡 | ✅ le follet suit dès la Grotte, **double action** (joueur : attaque / vitesse d'attaque / déplacement — monstres : ralentissement / dégâts) et **seule source de lumière la nuit** (**`rayon_lumiere` = 110 px logiques, validé** ; la lumière est collée au follet, seul halo statique = level design, 2026-09-16) · ① stats propres · ③ capacités d'exploration · ④ équipable ? · ✅⑤ changement débloqué après Boss 1 · ⑥ lien comportemental (M2+) · ⑦ alignement (M2+) |
...
| D14 | **Énigmes / puzzles** | 🟡 | ✅① types réutilisables + pièces uniques · ② rôle du compagnon dans la résolution · ③ difficulté / indices · ④ blocage dur ou contournable · ⑤ piste « DaVinciCode » |
| D15 | **Zones / maps / spawn** | 🟡 | ✅① tuiles réutilisables + layout main-made + décor procédural déterministe · ② schéma d'une zone · ③ portails déclaratifs (patron V1 validé) · ④ tables de spawn par zone · ⑤ densité et repop · ⑥ conditions d'accès · ⑦ mode Chaos/Normal : repris en V2 ou abandonné |
...
| D19 | **Survie** | 🟡 | ✅③ gel total hors session · **✅① nature des pénalités : `stat_effective = stat × (0,5 + 0,5 × jauge_moyenne)`, `jauge_moyenne = (nourriture + eau) / 2`, appliquée à Force et Agilité ; plancher (0,5) et pente par zone, en données (`scenes.json`), pour rendre zone de monstres et château plus punitifs que Maison/Poste avancé — décision Xav 2026-09-15** · ② vitesse de décroissance en jeu (Phase 3) · ④ mort possible par faim ? · ⑤ lien cuisine → buffs · ⑥ désactivable / mode détente |
...
| P4 | **Accessibilité** | 🟡 | ✅② éléments identifiables sans la couleur, dès la conception · ① taille minimale des cibles tactiles · ③ mode une main · ④ vitesse de texte / temps de lecture · **✅ ligne de DA : jamais de noir absolu — obscurité par scène (0,72 en grotte, ~0,9 pour les futures cartes « vides » éclairées par le seul follet) ; direction artistique validée en jeu le 2026-09-16 sur le catalogue `visuels.json` (une seule fonction de rendu)** |
...
| 2026-09-15 | D6① | Buffs/débuffs + DoT + contrôles, dérivés logiquement de l'élément | La logique élémentaire tient lieu de règle de design ; les synergies sont actées le même jour (D6③) |
...
| 2026-09-15 | C3③ | Mapping manette : ABXY = attaque + 3 skills, gâchette = consommable, LB = interact, Start = menu (n'ouvre qu'un menu fermé), B = retour dans toute UI ; croix directionnelle réservée à de futures actions secondaires, jamais `MOVE` | Cohérence manette-référence (C0) ; validé à la manette réelle le 2026-09-16 |
| 2026-09-15 | C3⑦ | Hot-swap manette ↔ clavier validé en jeu | Confirme que la couche d'input abstraite absorbe le changement de périphérique sans redémarrage |
| 2026-09-16 | C3② | Mapping tactile : choix du follet au stick, pas au tap direct — dette assumée jusqu'aux compétences (Phase 4) | Éviter de dupliquer un second mécanisme de hit-test tactile pour un seul écran, le tactile alimente déjà `move`/`attack` |
| 2026-09-15 | C7② | Résolution logique 480 × 270, cadrage à échelle entière, rendu net à la résolution physique (DPR pris en compte), le jeu n'est pas en pixel art | Lisibilité du HUD et du texte ; validé sur capture le 2026-09-15, rendu net validé le 2026-09-16 |
| 2026-09-15 | D2③ | Définition d'une synergie : un effet sur le joueur + un effet sur le monstre engagé dans l'aura, par élément, en données | Débloqué par D6③ (table des synergies écrite) |
| 2026-09-15 | D3② | Schéma d'arme : ajout de `portee {min, max}` en tuiles — la portée de l'auto-attaque vient de l'arme, jamais d'une stat | Généralise sans code aux armes à distance futures (arc `[2;4]`) |
| 2026-09-15 | D6③ | Table des synergies élémentaires écrite et actée (`02_grotte.md` §3.4) | Débloquait D2③ ; valeurs provisoires à équilibrer en jeu |
| 2026-09-16 | D11 | `rayon_lumiere` = 110 px logiques validé ; lumière collée au follet, seul halo statique = level design | Confirmé en jeu à la manette réelle |
| 2026-09-15 | D19① | Formule de survie actée : `stat_effective = stat × (0,5 + 0,5 × jauge_moyenne)`, `jauge_moyenne = (nourriture + eau) / 2`, appliquée à Force et Agilité ; plancher et pente par zone, en données | Équilibre naturel, une seule valeur à lire ; rend zone de monstres/château plus punitifs que Maison/Poste avancé |
| 2026-09-16 | P4 | Ligne de DA : jamais de noir absolu — obscurité par scène (0,72 grotte, ~0,9 cartes « vides ») ; direction artistique validée en jeu sur le catalogue `visuels.json` | Cohérence du rendu sur toutes les scènes futures, une seule fonction de rendu (`dessinerVisuel`) |
| 2026-09-16 | Phase 1 | Cinématique d'intro : oui, ≤ 10 s, non skippable, prérequis = catalogue visuel verrouillé | Évite d'animer des silhouettes qui changeront encore |
| 2026-09-16 | D5 | Feedback de combat : anneau + barre de PV du monstre + flash blanc au coup | Lisibilité du combat sans sprite ni animation squelettique |
| 2026-09-16 | C7⑤ | Anti-spam dialogue : frame consommée + machine à écrire + armement, `ATTACK` reste la confirmation | Empêche qu'un dialogue se referme sans avoir été lu pendant un spam de bouton |
| 2026-09-16 | P4 | Couleur neutre du héros avant le choix : gris moyen désaturé + contour clair — validé en jeu | Lisibilité du héros sur le voile d'obscurité avant que le follet ne le teinte |
