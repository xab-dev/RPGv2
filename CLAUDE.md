...
Phases validées : 0 (Socle technique), 1 (La Grotte), 1b (polish, DA validée), 2 (Région Maison, première marche, `specs/03_maison-exterieur.md`) et **3** (Maison, intérieur & systèmes de camp, `specs/04_maison-interieur.md`) — **Phase 3 close le 2026-09-17**, verdict Xav en jeu : tous les points testés, bon (détail complet : `docs/archives/INDEX.md`). Chantier courant : `specs/05_construction-stations.md` v1.0.1 (placement libre des stations dans la Maison), codé en entier le 2026-09-17. Quatre validations/diagnostics à ce jour, tous corrigés et testés headless le jour même de leur découverte : l'écran-liste resté affiché pendant tout le placement (`docs/archives/JOURNAL_2026-09-17_bandeau-placement.md`), le menu Pause resté ouvert de façon invisible à l'entrée en placement (`docs/archives/JOURNAL_2026-09-17_diagnostic-menu-ouvert-placement.md`), puis le contrat « ouvert » unifié pour tous les contrôleurs de `ui/menu.js` (`docs/archives/JOURNAL_2026-09-17_ecrans-orphelins.md`) — Xav a ensuite validé Construction au clic/tactile (première validation tactile réelle du jeu, jouable) mais reproduit le MÊME symptôme apparent (menu Pause qui réapparaît) à la manette **et** au clavier seul, cause racine DISTINCTE (une déduction après coup dans `creerEcranListeGenerique#traiterInput`, pas le contrat « ouvert ») réglée en journal courant par un événement `onAnnuler` explicite — **le ressenti manette/clavier reste encore à revoir en navigateur par Xav**, voir « Critère de passage courant ».
...
| Fermeture d'un écran de `creerEcranListeGenerique` : événement explicite `onAnnuler` (câblé sur `verbeAnnuler`) plutôt qu'une déduction après coup sur l'état du contrôleur — *2ᵉ classe de bug distincte* du contrat « ouvert » ci-dessus, cette fois entre le chemin clic (sain) et le chemin verbe (manette/clavier cassés) | 2026-09-19 | journal courant |
...
- **Mesure réelle du temps de frame / fps** (plancher mobile jamais mesuré, seulement borné fonctionnellement par `selectionnerTuilesVisibles`) — 2026-09-16, même archive ; **rendu signalé « un peu laggy » par Xav sur tactile réel** (2026-09-19, `SD_construction-parite-clic-verbe_2026-09-19.md` §5) — même dette, pas encore diagnostiquée, ne pas guess-fix.
...
- **Validation manuelle de la Construction encore due par Xav — partiellement close le 2026-09-19** : Xav a validé Construction au clic/tactile en jeu (première validation tactile réelle du jeu, jouable). **Reste dû : le ressenti manette/clavier seul**, jamais revu en navigateur depuis le correctif de parité clic/verbe (journal courant) — états 29-32 et 31bis de `docs/CHECKLIST_visuelle.md` toujours à capturer (fantôme vert/rouge, station tournée, entrée de menu contextuelle, bandeau de placement).
...
**Chantier courant — Construction** (spec `specs/05_construction-stations.md` v1.0.1), extrait de la Phase 3 (ex-palier F). Code livré en entier le 2026-09-17 le jour même de la clôture de la Phase 3 : `placement.js` (grille, rotation, `poseValide`, BFS de praticabilité du couloir), `structures.js#tournerEmpreinte`/`empreinteAbsoluePuzzle` (une seule fonction pour la collision ET le rendu), sauvegarde v5 (`maison.stations`), menu Pause « Construction » contextuel, fantôme de pose vert/rouge (+ marqueur de forme distincte). Trois validations/diagnostics le 2026-09-17 : (1) l'écran-liste restait affiché PENDANT le placement (housing à l'aveugle) — corrigé par un bandeau DOM en filigrane (`docs/archives/JOURNAL_2026-09-17_bandeau-placement.md` → v1.0.1) ; (2) le menu Pause restait ouvert de façon invisible à l'entrée en placement — corrigé (à tort, en apparence) en fermant explicitement le contrôleur de premier niveau dans `ouvrirPlacementConstruction` (`docs/archives/JOURNAL_2026-09-17_diagnostic-menu-ouvert-placement.md`) ; (3) ce 2ᵉ correctif cassait le sens RETOUR (liste → menu Pause) — cause racine (2 contrats différents pour « ouvert » sous le même `menu.estOuvert()`) établie par une session de lecture seule (`docs/CARTE_cycle-de-vie-ui_2026-09-17.md`), réglée en unifiant le contrat de TOUS les contrôleurs de `ui/menu.js` (`creerControleurMenu#element` : booléen ET DOM visible, `docs/archives/JOURNAL_2026-09-17_ecrans-orphelins.md`).

**2026-09-19 : 4ᵉ diagnostic, cause racine DISTINCTE des 3 précédents.** Xav a validé Construction en jeu au clic ET au tactile (première validation tactile réelle du jeu, jouable) — mais reproduit le MÊME symptôme apparent (menu Pause qui réapparaît, stick qui navigue dedans au lieu de déplacer le fantôme) à la manette **et** au clavier seul. Ce n'était donc pas (ou plus) le contrat « ouvert » du 17/09 soir : `creerEcranListeGenerique#traiterInput` DÉDUISAIT « fermé par B/skill_3 » du seul fait que le contrôleur local était fermé après `traiterInput(etat)`, alors qu'une action choisie par ATTACK (sélectionner une station → `demarrerConstruction` → `fermerSansCallback()`) pouvait aussi le fermer, sans jamais vouloir rappeler `onFermer`. Le clic, qui appelle `toutes[i].action()` directement sans jamais passer par cette déduction, ne pouvait pas se tromper — d'où souris/tactile sains, manette/clavier cassés identiquement. Corrigé (`src/ui/menu.js`, journal courant) en donnant à `creerControleurMenu` un événement explicite `options.onAnnuler` (déclenché UNIQUEMENT par `verbeAnnuler`, jamais déduit) ; la branche `else` qui déduisait a été supprimée, pas contournée. Test de parité clic/verbe générique et data-driven (`tests/test_sd_construction_parite_clic_verbe_2026-09-19.js`) : séquence composée S1→S4 jouée sur deux bancs indépendants (clics purs / verbes purs, aucun appel direct à `entree.action()`), **rouge confirmé sur HEAD avant correctif** (échoue exactement à la sélection de la 1ère station, comme prédit) puis vert après ; plus une boucle générique sur CHAQUE entrée de CHAQUE écran (Poche/Craft/Coffre/Stats/Construction), entrées découvertes depuis le DOM réel, jamais codées en dur. `node tools/run_tests.js` : 59 fichiers, tous verts. **Validation manette/clavier par Xav encore due** — jamais revu en navigateur depuis ce correctif, voir Dette et `docs/CHECKLIST_visuelle.md` états 29-32/31bis.
