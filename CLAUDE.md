...
**Chantier en cours : `specs/08_menus-cartes.md` — les menus en grille de cartes (`D-43`).** Spec **par paliers, un palier par session**, branche `menus-cartes`, un commit par étape, chacun retirable seul. Palier A (l'écran de référence) : livré le 20/09 ; Xav l'a regardé le soir même (« tout à l'air bon ») et a **demandé d'aller au bout de la spec dans la nuit**. **Les trois paliers sont livrés sur la branche `menus-cartes`, poussée (sauvegarde autorisée par Xav pour cette branche, jamais `main`), NON fusionnée — la fusion est la sienne** : B (une seule pile pour tout le menu, `MENU` ferme tout), C (Poche, Stats, Coffre, Craft et Construction en « maître-détail » : il n'existe plus d'écran de liste), puis une passe de polish. **Rien de cette nuit n'a été vu sur un écran allumé ni joué à la manette ou au doigt** (vérifié sous Chrome sans fenêtre, aux deux tailles) : validations dues `V-27`, `V-28`, `V-29` ; choix de conception à confirmer `Q-36`, `Q-39`.
...
5 bis. ~~**`specs/08_menus-cartes.md`** (`D-43`)~~ — **les trois paliers et le polish sont livrés** sur la branche `menus-cartes` (poussée, non fusionnée). Restent à Xav : la fusion, puis `V-27`, `V-28`, `V-29`, et confirmer ou réviser `Q-36` et `Q-39`.
