...
| P4 — le ton du laboratoire | `47434ef` | Retour de Xav sur `V-121` (« c'est parfait, on garde cette base » ; police calligraphique, écriture elfique, ton du README). `src/polices.js` + `fonts/` (Almendra, Uncial Antiqua, SIL OFL 1.1, embarquées), chargées par `demarrerJeu` (plafond 3 s, repli `serif`) ; `.ttf` servi en `font/ttf` et marqué binaire ; `glyphe` facultatif (signes alchimiques du README, tracés) ; textes réécrits FR/EN (`specs/12` v1.1.0 §4) ; colonne de texte à 410 px logiques (mesuré : le plus long vers du README en demande ~400). Pixels revus sous les trois profils |
