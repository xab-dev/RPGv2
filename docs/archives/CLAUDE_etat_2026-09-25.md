...
# RPG V2 — `CLAUDE.md` tel qu’au 25/09, avant le ménage du contexte

Recopié **tel quel**, en entier, le 25/09 avant sa réécriture (`docs/NS_menage-contexte-claude_2026-09-25.md`, `DOC-10`). Source de vérité si un doute naît sur ce que disait le fichier : rien de ce qui suit n’a été révisé par le ménage, seulement déplacé ou résumé. Les décisions datées du 23 au 25/09 sont aussi dans `decisions_archives.md`, l’arborescence commentée dans `docs/ARCHITECTURE.md`.
...
**Dernière publication : `v0.8.0`, le 25/09.** `annexe-1` (spec 14) et `torche-vacillement` (`D-218`, `D-219`, SD_torche) ont été fusionnées dans `main`, chacune par un commit de fusion, et poussées à la demande de Xav. Puis `avertissement-epilepsie` (`D-220`, `D-221`), poussée sans release : elle ira dans la suivante. Puis `profondeur` (`D-222` la profondeur, `D-223` le fondu d'un passage, `D-224` la hitbox des arbres, validées en jeu), fusionnée et poussée le 25/09 à la demande de Xav, sans release non plus. Puis `polish-libre-2026-09-25` (`D-225`, `D-29`, `D-44`, `D-226`, `D-41`, `D-10`) et `outil-dettes` (`D-227`, `npm run dettes`), fusionnées et poussées le 25/09. Puis `orientation-heros` (`D-229`, `D-247`, `D-248`, `D-249` : le héros regarde en huit directions, l'Onde se vise au curseur et au doigt), fusionnée et poussée le 25/09 au soir, sans release. Puis `triche` (`D-250`, `?cheat=phenom`), fusionnée et poussée le 25/09 dans la nuit pour les tests au téléphone, sans release ; les branches de travail fusionnées ont été supprimées à la demande de Xav. Un correctif tenu à part d'une spec en cours part de `main` sur sa propre branche, et prend ses identifiants **après** ceux de la branche en cours, pour qu'aucun doublon n'apparaisse à la fusion.
...
| **Le héros regarde, par une PIÈCE de son visuel** : une primitive porte `piece`, le visuel déclare ce que chaque direction lui fait (`orientations` : cachée, ou décalée et resserrée) ; la pose de face n'est jamais déclarée, c'est le dessin validé. La direction est un état d'affichage (`orientation.js`), jamais sauvegardé, et la **visée ne dépend pas du regard**. **Huit directions** (Xav), la pointe de la capuche penche à l'opposé du regard (`cisaillement` autour de sa base) | 2026-09-25 | `D-229`, `D-249`, `Q-167` |
| **Une compétence se vise** : au curseur (la souris au clavier, le stick droit à la manette), ou au doigt en glissant depuis son bouton, qui part alors au relâchement ; sans visée, la cible automatique du follet. `competences.js#pointVise` est LE point de résolution d'un tir ; le gameplay reçoit un point d'écran ou une direction (`lireVisee`), jamais un périphérique | 2026-09-25 | `D-247`, `D-248`, `Q-167` (défauts : `Q-168`, `Q-169`) |
