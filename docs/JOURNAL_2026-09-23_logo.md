...
| L4 — le symbole à la montée de niveau | `08e61dd` | `main.js` : `logoNiveauMs`, né sur la même détection que l'éclat « Niv. N », au-dessus du héros (il le suit), après l'obscurité et avant le HUD ; discret (`effet_logo_niveau` : 22 px, alpha 0,7, ~1,6 s — PROVISOIRE). Pixels NON capturés (déclencher un niveau sous Chrome sans fenêtre demande une récolte scriptée) : même fonction de dessin que l'ouverture, vérifiée, mais la taille et la place se jugent en jeu ; `tests/test_l4_logo_niveau` |
| L5 — le suivi | (ce commit) | `DOC_suivi-dettes.md` : `Q-115` (le symbole sur les armes, quand elles arrivent — rien de fait), `Q-116` (ce qui est retenu par défaut en jeu), `Q-117` (une version réduite pour les très petites tailles) ; `V-118` à `V-120` (l'ouverture, la montée de niveau, le README). `CLAUDE.md` : `src/logo.js` et `images/logo/` dans l'architecture |

**Au prochain ménage** : archiver ce journal ; rien à reporter dans les décisions verrouillées, sauf si Xav tranche `Q-116`.
