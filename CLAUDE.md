...
- **Branches ouvertes**, rien fusionné ni poussé : la nuit du héros (26/09) — `heros-simplification` (`D-262`, `poses.js`) → `heros-polish` (`D-263` à `D-267`) → `heros-animation` (**spec 16** paliers A à D : `D-268` à `D-271`, `D-273` le pas qui suit la vitesse ; `V-182` à `V-190` validés, `V-191` à voir). Et `reinit-exporter` (depuis `main`, `D-274` « Exporter d'abord » ; `V-192` à voir). À reprendre : `D-261` (le profil, à dire par Xav), `D-272`.
