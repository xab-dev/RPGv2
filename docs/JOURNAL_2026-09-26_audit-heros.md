...
| `(ce commit)` | D-294 | **Couche « traits ».** Variante C de Xav (cumulative) : liseré de lumière sur l'épaule, filet d'ombre bas-droit, trois plis fins, l'ourlet de la capuche ; 34 → 43 primitives, données seules. Xav sur `V-202` : « il n'y a pas un seul angle où je vois un défaut, très bien ». Sentinelle Moyen : ×1 0,85 ms, 0 frame > 20 ms ; ×6 5,68–5,97 ms, 4–5 frames > 20 ms (fin du palier C : 5,39 / 1) — dans le bruit des passages ×6, à surveiller au banc complet. À voir : `V-203` |
