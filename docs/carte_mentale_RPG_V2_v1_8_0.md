...
| **Sandbox et monde ouvert** (26/09) | « ajouter à la carte mentale dans la vision long terme du jeu : notion de 'sandbox' et 'openworld' » | la Maison qui s'agrandit par annexes et tunnels plutôt que par cartes successives ; aller et venir librement (la boucle de 5 min, les allers-retours au coffre) ; des coffres qui se remplissent de ce que le joueur a choisi ; le follet agentique qui travaille où on le pose. **Tension à tenir** avec la narration diffuse et le périmètre M1 fermé (§0) : un monde ouvert se prépare dans l'architecture (scènes chargées à la demande, états du monde en flags), il ne s'ouvre pas avant une spec |
...
| **D1⑧** 🟢 | Rôle d'Esprit | **Réserve de lancement des compétences, rien d'autre.** Esprit ne fait pas scaler les dégâts. Conséquence à trancher en D2⑦. **Confirmé le 2026-09-23** (l'idée « Esprit = alignement » du 22/09 est abandonnée : l'alignement est une stat cachée distincte, D11⑦) ; dérivée `réserve d'esprit` posée par `D-141`. **Révisée le 2026-09-24 (Xav, `specs/14` §0 B1)** : Esprit **amplifie** les compétences — un coefficient sur la Force des compétences et une hâte sur leur charge et leur recharge ; il ne fait jamais de dégâts sans Force. |
...
### Vocabulaire de la Région Maison (figé le 2026-09-19, **révisé le même jour** — c'est cette version qui fait foi)
...
| **Zone sûre** | Maison + Jardin, **et un rectangle autour de la sortie de la Grotte** (point de retour après une mort). Aucun monstre n'y apparaît, aucun n'y entre : il fait demi-tour. |
...
| **Champs** | **Deux grandes zones en L**, au nord et au sud de la bande centrale, à l'est de la Forêt. C'est là que le Chaos s'installe la nuit, et là qu'une ferme viendra plus tard. |

**Toutes les zones se décrivent en rectangles**, au format `zones` déjà présent dans `scenes.json` — jamais en centre + rayon. *Révise le tableau du 2026-09-19 matin* (« angles opposés à la Forêt », « le Jardin est la zone sûre », « Campagne = le reste ») : Xav avait dit « les angles », il a précisé.

### Arc de progression de la carte Maison (2026-09-19)

Le seuil « accès à la 1ère zone de monstres gaté par niveau ~5 » est **abandonné** — *révise une décision verrouillée du 2026-09-15*. La carte suivante s'ouvre quand la carte Maison est **épuisée**. Conséquence : les *systèmes* prévus en Phase 4 (armes, équipement, compétences, tables d'apparition) arrivent d'abord **sur la carte Maison** ; la *carte* de la Phase 4 vient après.

| Niveau | Ce qui s'ouvre sur la carte Maison | Statut |
|---|---|---|
| 5 | Zone de Chaos nord-est (la nuit) | **décidé** — périmètre de `specs/07_chaos-nocturne.md` v1.1.0 |
| 10 | Zone de Chaos sud (la nuit) | **décidé** — micro-ticket données seules, plus tard |
| 15 | Apparitions éparses en Forêt et dans les Champs (la nuit) | **décidé** — *remplace* « quelques monstres épars en Forêt » dès le début : la Forêt reste vide avant 15 |
...
| 30 | Les compétences | idée — frotte contre D21① (voir la note ci-dessous) |
...
Note de lecture (Xav) : « fluidité » désigne le **ressenti des mouvements** (stick + follet en orbite), validé. La régularité de l'affichage (saccades) est un sujet distinct — mesurée depuis le 2026-09-19, suivie dans `docs/DOC_suivi-dettes.md` (`D-01` à `D-03`, registre des relevés §6).
...
| D21 | **Progression & déblocages** | 🟡 | ✅① double axe XP→stats / jalons→capacités · ✅ **sources d'XP : combat et craft** · ✅ **le gating de la 1ère zone par niveau ~5 est abandonné** (2026-09-19) : la carte Maison s'épuise d'abord (arc de progression §3bis), la carte suivante s'ouvre vers Nv. 40-50 · ⑦ **`Q-18`** : compétences au Nv. 30 vs double axe D21① — piste « le niveau ouvre le lieu, le lieu donne la capacité », non tranchée · ② courbe d'XP · ③ points de stat par niveau : attribution libre ou automatique · ④ liste des jalons de M1 · ⑥ rythme des boss |

**D19 / D10 (jardin)** — cadre général tranché le 2026-09-19, détail encore ouvert :

> ⑦ **Intrusion nocturne du Chaos dans la Région Maison — tranchée en partie le 2026-09-19** (`NS_decisions-playtest_2026-09-19.md`) : *révise* « aucun monstre, ton chill » de `03_maison-exterieur.md` §5. Motif : la carte s'est révélée bien plus grande que prévu, et la nuit au seul follet est l'ambiance la plus forte du jeu à ce stade. Cadre acté, **précisé le 2026-09-19 (revue des dettes)** : la nuit seulement ; **par paliers de niveau déclarés en données** — Nv. 5 zone de Chaos au nord-est, Nv. 10 zone sud, Nv. 15 apparitions éparses en Forêt et dans les Champs (*révise* « quelques monstres épars en Forêt » dès le début : **la Forêt reste vide avant 15**) ; monstres présents jusqu'à la fin de la nuit ; **un monstre qui entre en zone sûre fait demi-tour** — condition sur la position du monstre, jamais sur celle du joueur. Comportement : **« un domaine, pas un piquet »** — errance dans un domaine fait de zones, poursuite bornée depuis le point de repérage, désintérêt de quelques secondes après un abandon, anti-blocage après ~1 s sans avancer ; un monstre ne sort de son domaine que si le joueur l'y attire. Spec écrite : `specs/07_chaos-nocturne.md` v1.1.0, qui ne livre que le système et le palier 1, **un palier par session**. `[OUVERT]` restant : **la destruction des plantations par les monstres n'est pas tranchée** — le jardinage n'existe pas encore, donc rien à détruire pour l'instant ; à retrancher quand D10 (jardinage) sera écrit.
...
| 2026-09-19 | §3bis | **Vocabulaire de la Région Maison révisé** : Forêt = côté ouest · Campagne = bande centrale **neutre** · Champs = deux grandes zones en L (nord et sud) · zone sûre étendue à un rectangle autour de la sortie de la Grotte · **toutes les zones en rectangles**, format `zones` de `scenes.json` | Xav avait dit « les angles » et a précisé ; le format rectangle existe déjà et évite d'inventer un centre + rayon |
| 2026-09-19 | D21 | **Le gating de la 1ère zone par niveau ~5 est abandonné** — *révise une décision verrouillée* : la carte suivante s'ouvre quand la carte Maison est épuisée (Nv. 40-50, provisoire) ; les systèmes de la Phase 4 arrivent d'abord sur la carte Maison | La carte Maison s'est révélée bien plus grande que prévu ; la quitter au Nv. 5 gâcherait le lieu déjà construit |
