...
- **Dernière publication : `v0.8.83`** (26/09, `main` et le tag poussés ; dernière release : `v0.8.0`). Depuis `v0.8.0` : épilepsie (`D-220`, `D-221`), profondeur (`D-222` à `D-224`), polish libre, `npm run dettes`, orientation et visée (`D-229`, `D-247` à `D-249`), triche `?cheat=phenom` (`D-250`), dettes `D-243`, `D-244`, `D-246` (`V-171` à `V-173` à voir), le ménage du contexte (`DOC-10`), le joystick en laisse et bornée (`D-138`, `D-251`, `Q-178`, `Q-179`), et le héros redessiné (`D-252` à `D-260` : la capuche pliée sur l'axe dans les huit vues, l'œil en globe au fond de sa capuche, les proportions harmonisées, le regard et l'ouverture de côté ; `V-176` validé à l'oral, `V-177` à `V-181` à voir).
- **Branches ouvertes** : la nuit du héros (26/09), rien fusionné ni poussé — `heros-simplification` (`D-262`, les poses simplifiées, `poses.js`) → `heros-polish` (`D-263` à `D-267` : volume, jonctions, lumière et reflets de l'œil) → `heros-animation` (**spec 16**, paliers A à C : le héros à tout angle, qui tourne en jeu, le souffle et le pas ; `D-271`, les dégradés gardés). `V-182` à `V-190` à voir ; le banc du tour : `tools/banc_tour.html`. À reprendre aussi : `D-261` (le profil), `D-272`.
